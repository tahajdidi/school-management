...
class UserService {
    ...
    protected $batch;
    protected $st, $st2;
    protected $parentsstudent;
    
    public function __construct(User $user,ParentsStudent $parentsstudent, StudentInfo $student_info, DB $db, Batch $batch){
        $this->user = $user;
    ...
    public function getstudentbyparents($id){
        $students= $this->parentsstudent->where('parents_id',$id)->get();
        $ret=array();
        foreach($students as $student)
        {
            $p=$this->getStudentsbyid($student->student_id);
            array_push($ret, $p);
    ...
    public function getStudentWithParents(){
        return $this->parentsstudent->with('student','parents')
       ->paginate(1000);

        
    }
    public function getParentsByCode($parents_code){
        $parents= $this->user->where('code', $parents_code)
                ->get();
        return $parents[0]->id;
            }
    public function getStudentbystudentcode($student_code){
        $student= $this->user->where('student_code', $student_code)
                ->get();
        return $student[0]->id;
            }
    public function isListOfParents($school_code, $student_code,$teacher_code){
        return !empty($school_code) && $student_code == 1 && $teacher_code == 1 ;
    }
    ...
    public function createstudentinfo_existparents($request, $id,$parents){
        ...
        $info->father_name =$parents->father_name;
        $info->father_phone_number =$parents->father_phone_number;
        $info->father_national_id = $parents->father_national_id ;
        $info->father_occupation = $parents->father_occupation;
        $info->father_designation = $parents->father_designation;
        $info->father_annual_income =  $parents->father_annual_income;
        $info->mother_name = $parents->mother_name;
        $info->mother_phone_number = $parents->mother_phone_number;
        $info->mother_national_id = $parents->mother_national_id;
        $info->mother_occupation = $parents->mother_occupation;
        $info->mother_designation = $parents->mother_designation;
        $info->mother_annual_income = $parents->mother_annual_income;
...
->get();            }
    ...
    public function storeparentsstudent($parents_id,$student_id){
        $t = new $this->parentsstudent;
        $t->student_id = $student_id;
        $t->parents_id = $parents_id;
        $t->save();
        return $t;
    }
    ...
    public function storeparents($request){
        ...
        $tb->email = (!empty($request->parents_email)) ? $request->parents_email : '';
        $tb->password = bcrypt($request->parents_password);
        $tb->role = 'parents';
        $tb->active = 1;
        $tb->school_id = auth()->user()->school_id;
        $tb->code =$request->father_national_id;
        $tb->phone_number = $request->father_phone_number;
        $tb->pic_path = (!empty($request->pic_path)) ? $request->pic_path : '';
        $tb->verified = 1;
        $tb->save();
        return $tb;
