...
use App\Http\Requests\User\CreateParentsRequest;
use App\Http\Requests\User\UpdateUserRequest;
use App\Http\Requests\User\CreateAdminRequest;
use App\Http\Requests\User\student_with_exist_parents_request;
...
class UserController extends Controller
{
    ...
    public function checkID(Request $request){
        $parents = StudentInfo::query()
       ->where('father_national_id',$request->parents_id)
        ->get();
        ...
        session([
            ...
            'register_role' => 'student_with_exist_parents',
            'register_rolee' => 'student with exist parents',
            'register_parents' => $parents[0],
        ]);
        return redirect()->route('register');
        //return $parents[0];
    }
    public function parents_message(){
       /* foreach ( $this->userService->getStudentWithParents() as $value) {
            var_dump($value); 
            die();
        }*/
        return  $this->userService->indexView('notification.parentsnotification', $this->userService->getStudentWithParents());}
    public function selectID(){return view('selectID');}
    ...
    public function index($school_code, $student_code, $teacher_code){
        ...
        if ($this->userService->isListOfParents($school_code, $student_code,$teacher_code))
           return  $this->userService->indexView('list.parents-list', $this->userService->getStudentWithParents());
        else if($this->userService->isListOfStudents($school_code, $student_code))
       ...
       // else if($this->userService->isListOfParents($school_code, $teacher_code))
         //   return $this->userService->getStudentWithParents();
    ...
    public function redirectToRegisterParents()
    {
     
        session([
            'register_role' => 'parents',
     ...
     * @return \Illuminate\Http\Response
     */
    public function store_student_with_exist_parents(student_with_exist_parents_request $request)
    { $parents= session('register_parents');
        
    $parents_id=$this->userService->getParentsByCode($parents->father_national_id);
        DB::transaction(function () use ($request) {
            ...
            $this->userService->storeparentsstudent($parents_id ,$tb->id);
            $this->userService->createstudentinfo_existparents($request ,$tb->id,$parents);
    ...
    public function storeParent(CreateParentsRequest $request)
    {   
        DB::transaction(function () use ($request) {
            $student_id = session('student_id');
           $password = $request->password;
           $tb = new $this->user;
           $tb->name = $request->father_name;
           $tb->email = (!empty($request->email)) ? $request->parents_email : '';
           $tb->password = bcrypt($request->parents_password);
           $tb->role = 'parents';
           $tb->active = 1;
           $tb->school_id = auth()->user()->school_id;
           $tb->code =session('father_code');
           ...
           $tb->verified = 1;
           $tb->phone_number =$session('father_phone_number');
           $tb->save();
            $this->userService->storeparentsstudent($tb->id,$student_id);
    ...
    {  
       DB::transaction(function () use ($request) {
        $password = $request->password;
        $tb = $this->userService->storeStudent($request);
         $tb1=$this->userService->storeparents($request);
         $this->userService->storeparentsstudent($tb1->id,$tb->id);
            try {
